...
void setPerspective()
{
	//Ch6 P.131 like eyeview 
	//near is small, far is big
	cur_proj_mode = Perspective;
	// * PI / 180 because unit(���) of tan is degree
...
void RenderScene(void) {
	...
	// 3D model in solid or wireframe mode
	if (isWireframeMode)
	...
	//pass MVP to GPU's register
	glUniformMatrix4fv(iLocMVP, 1, GL_FALSE, mvp);
	//pass models to GPU's memory
	glBindVertexArray(m_shape_list[cur_idx].vao);
	//Before DrawArray, all we do are to set GPU
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	if (yoffset > 0) {
		...
		case ViewEye: 
			main_camera.position.z -= 0.035;
			setViewingMatrix();
			break;

		case ViewCenter: 
			main_camera.center.z += 0.35;
			setViewingMatrix();
			break;

		case ViewUp: 
			main_camera.up_vector.z += 0.75;
			setViewingMatrix();
			break;
		}
	}
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (mouse_pressed) {
		switch (cur_trans_mode) {
		case GeoTranslation: 
			models[cur_idx].position.x += delta_x * 0.002;
			models[cur_idx].position.y -= delta_y * 0.002;
			break;

		case GeoScaling: 
			models[cur_idx].scale.x -= delta_x * 0.001;
			models[cur_idx].scale.y -= delta_y * 0.001;
			break;

		case GeoRotation: 
			models[cur_idx].rotation.x -= PI / 180.0 * delta_y * 0.1;
			models[cur_idx].rotation.y -= PI / 180.0 * delta_x * 0.1;
			break;
...
void initParameter()
{
	...
	proj.nearClip = 0.001;
	proj.farClip = 100.0;
	//Range of view , Unit:degree, not pi
	proj.fovy = 80;
...
void setupRC()
{
	...
	for (int cur_idx = 0; cur_idx < model_list.size(); ++cur_idx)
	{
...
int main(int argc, char **argv)
{
	...
	// create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "110062619 HW1", NULL, NULL);
	if (window == NULL)
	{
